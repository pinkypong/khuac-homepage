...
## Gemini 안정성 수정 (2026-09-13)

- 기본 공급자는 Gemini Developer API입니다. `GEMINI_API_KEY`를 사용하며, 예전 Vertex 서비스 계정이 남아 있어도 자동으로 Vertex를 선택하지 않습니다. Vertex는 `GEMINI_PROVIDER=vertex`를 명시했을 때만 사용합니다.
- 모델은 `GEMINI_MODEL`로 설정합니다. 이번 로컬 실호출은 기존 기본값 `gemini-3.6-flash`로 확인했습니다.
- Gemini 호출별 총 제한 시간은 재시도를 포함해 60초입니다. 네트워크 오류와 429/500/502/503/504는 최대 3회 시도하며 Retry-After를 반영합니다. 지역 제한, 인증, 잘못된 요청, 결제 크레딧 소진은 반복하지 않습니다.
- 기존 코드에 작은 출력 토큰 제한은 없었습니다. 대신 일반 답변 3~5문장 지시, 질문 200자 자동 절단, DB에 등록된 장소만 코스 검색하는 조건이 있었습니다. 문장 제한을 제거하고 입력은 2,000자까지 명시적으로 검증하며, 미등록 장소도 검색합니다.
- 코스 검색 본문과 출처는 JSON 추출 실패 시에도 반환합니다. 코스 카드에는 거리, 소요시간, 난이도를 표시하며 누락된 값은 추측하지 않습니다.
- 지도는 검색 경유지를 연결한 참고도입니다. 실제 등산로 좌표를 복원하는 기능은 아니며, 정확한 등산로는 기존 GPX/등산로 등록 기능으로 관리합니다.

검증: 자동 테스트 74개 통과. 로컬 Developer API에서 `관악산 등산 루트`는 47.1초에 4개 코스, `관악산 등산 루트 추천해줘`는 42.6초에 3개 코스를 반환했습니다 (검색+구조화 합계). 두 질문이 다르게 분류되는 문제는 재현되지 않았습니다. 원래 운영 오류의 로그를 이번에 확보하지 않았으므로 특정 단일 원인으로 확정하지 않습니다.

운영 주의: 과거 코드 주석의 `User location is not supported`는 프롬프트/JSON 문제가 아닌 API 접속 지역 문제입니다. 지역 오류 발생 시 지원되는 지역에서 실행되는 백엔드 구성이 필요할 수 있습니다. 이번 작업은 로컬 수정이며 배포 및 Cloudflare 운영 환경 실호출 검증은 포함하지 않습니다.

공식 오류 처리 참고: https://ai.google.dev/gemini-api/docs/troubleshooting

## 추론 강도 (2026-09-13)

코스 검색은 `thinkingLevel: low`, 검색 결과 JSON 추출은 현재 기본 모델 Gemini 3.6 Flash에서 `minimal`로 설정합니다. 출력 토큰 제한을 줄이지 않으므로 답변을 강제로 잘라 비용을 줄이는 방식은 아닙니다. `minimal` 지원이 확인된 Flash 모델 외의 Gemini 3 모델로 변경하면 추출에도 `low`를 사용하며, Gemini 3 이외의 모델에는 이 설정을 보내지 않습니다. 일반 대화 답변의 추론 설정은 이번 변경 대상이 아닙니다.

변경 후 같은 질문 1회 실측: 검색 8.34초, JSON 추출 2.48초, 전체 12.81초 (초기 네트워크 오류 2회와 재시도 포함). 성공 응답의 사용량 합계는 입력 876 + 출력 1,295 = 2,171토큰이며 thoughtsTokenCount는 보고되지 않았습니다. 코스 3개, 소요시간, 경유지, 출처 8개를 반환했고 두 단계 모두 STOP으로 완료됐습니다. 변경 전 단일 측정 50.63초/12,540토큰 대비 감소했지만 검색 결과와 서버 상황에 따라 달라지는 단일 표본 비교입니다.
